...
index_list <- vector(mode = "list", length = nrow(hourly_file_name_lookup_tbl))
index_list[[1]] <- c("id")
all_hourly_data_list <- purrr::pmap(list(hourly_file_name_lookup_tbl$filename, hourly_file_name_lookup_tbl$metric, index_list),
                                   function(filename, metric, index_params) {
                                     print(metric)
                                     temp_tbl <- get_and_parse_hourly_data(filename, metric)
                                     temp_result <- add_to_db(temp_tbl, table_name = "hourly_weather_data",
